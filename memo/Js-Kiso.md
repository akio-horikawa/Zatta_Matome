...
オブジェクトはプロトタイプオブジェクトを継承する。オブジェクトリテラルを用いて生成されたものは全て同じプロトタイプオブジェクトを持つ。これをコード内で `Object.prototype` として参照可。 `new Object();` で生成したオブジェクトに関しても{}で生成したものと同じようにプロトタイプオブジェクトを継承する。 `new Array();` `new Date();` に関しても同じように `Array.prototype` `Date.prototype` を持つ。
