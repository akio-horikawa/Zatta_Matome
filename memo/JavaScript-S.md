...
関数呼び出し式によって関数かメソッドとして呼び出される。評価すると関数になる関数式を記述し、その後に丸括弧を続け、括弧内に0個以上の引数をカンマ区切りで記述する。関数式がプロパティアクセス式の場合、関数がオブジェクトのプロパティや配列の要素であった場合はメソッド呼び出し式になる。